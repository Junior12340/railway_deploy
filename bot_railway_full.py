...
def get_image_skip_keyboard():
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="⏭ Rasmsiz davom etish", callback_data="skip_image")]
        ]
    )
    return keyboard
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    if message.chat.type == "private":
        await message.answer(
            ...
            "🤖 Men online murojaatlar botiman.\n"
...
async def process_phone(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "   <i>Masalan: Qarashina shahri, Abdulla Qodiriy ko'chasi 123-uy</i>",
        parse_mode="HTML"
    )
...
async def process_text(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "⏭ Aks holda, <b>rasmsiz davom etish</b> tugmasini bosing.",
        parse_mode="HTML",
...
async def admin_reply_button(callback: CallbackQuery):
    ...
    await callback.answer(
        ...
        f"Masalan:\n reply Murojaatingiz ko'rib chiqilmoqda...",
        show_alert=True
    )
